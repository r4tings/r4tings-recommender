...
ggplot(dat, aes(x = rating, , y=method, fill=method, color=method)) +
  ...
  #  geom_vline(xintercept = 3, linetype= "solid", size=0.5, color="black") +
  #  annotate(x = 1.5, y = -Inf, label = "중간값", vjust = -2, geom = "text") +
  #  geom_vline(xintercept = 0, linetype= "solid", size=0.5, color="black") +
  #  geom_segment(aes(x = median(rating), y = 0.5, xend =0.1, yend =0.5), 
  #               arrow = arrow(length=unit(0.2,"cm")),
  #               color='black') + 
  geom_density_ridges(alpha=0.7, scale=0.8, stat="binline") + 
  #geom_density_ridges(alpha=0.2, scale=0.8, size=1.05) +
